    ...
    for layer in "${layers[@]}"; do
        for model in "${models[@]}"; do
            ...
            for config in "${configs[@]}"; do
                ...
                nohup python -u /home/guorui/workspace/dgnn/b-tgl/预处理/disk版本预处理.py --data=${d} --config="/raid/guorui/workspace/dgnn/exp/scripts/TGN-1.yml" --pre_sample_size=2000 &>/raid/guorui/workspace/dgnn/exp/res-${timestamp}/${d}/b-${block_size}-1-part.log &
                pid=$!
                memory_usage_file="/raid/guorui/workspace/dgnn/exp/res-${timestamp}/${d}/b-${block_size}-1-part-mem.log"
