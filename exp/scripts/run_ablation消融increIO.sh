    ...
    for layer in "${layers[@]}"; do
        for model in "${models[@]}"; do
            ...
            for config in "${configs[@]}"; do
                ...
                nohup python -u /home/guorui/workspace/dgnn/b-tgl/train纯IO测试.py --data=${d} --train_conf=${config} --config="/raid/guorui/workspace/dgnn/exp/scripts/${model}-b-${layer}.yml" &>../res-${timestamp}/${d}/b-${model}-${layer}-${config}_res.log &
                pid=$!
