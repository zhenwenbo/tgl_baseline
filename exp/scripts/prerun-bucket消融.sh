...
configs=("disk")
block_sizes=("2000" "4000" "6000" "8000" "10000" "20000" "40000" "80000" "100000" "200000" "400000" "600000")
config="disk"
layers=("1")
    ...
    for layer in "${layers[@]}"; do
        for model in "${models[@]}"; do
            ...
            for block_size in "${block_sizes[@]}"; do
                ...
                nohup python -u /raid/guorui/workspace/dgnn/b-tgl/train.py --pre_sample_size=${block_size} --data=${d} --train_conf=${config} --config="/raid/guorui/workspace/dgnn/exp/scripts/${model}-b-${layer}.yml" &>../res-${timestamp}/${d}/b-${model}-${layer}-${config}_res-${block_size}.log &
                pid=$!
