...
class TrainNegLinkSampler:
    ...
    # 做分区：将num_nodes随机分成k份
    def __init__(self, num_nodes, k = 8):
        self.part = []
        nodes = torch.randperm(num_nodes, dtype = torch.int32)
        per_node_num = int(math.ceil(nodes.shape[0] / k))
        left, right = 0, 0
        self.k = k
        for i in range(k):
            right += per_node_num
            right = min(right, nodes.shape[0])

            self.part.append(nodes[left:right])
            left = right
        self.num_nodes = num_nodes

    def sample(self, n, i = 0, cur_batch = 0):
        # res = np.random.randint(self.num_nodes, size=n)
        i = ((cur_batch * 2000) // self.part[0].size()[0]) % self.k
