...
class GeneralModel(torch.nn.Module):
    ...
    def __init__(self, dim_node, dim_edge, sample_param, memory_param, gnn_param, train_param, emb_buffer, combined=False):
    ...
    def forward(self, mfgs, neg_samples=1):
        ...
        # print(f"GCN层over, time: {time.time() - start:.4f}")
        if self.sample_param['history'] == 1:
            out = out[0]
        else:
            out = torch.stack(out, dim=0)
            out = self.combiner(out)[0][-1, :, :]
            # print(f"RNN层over, time: {time.time() - start:.4f}")
