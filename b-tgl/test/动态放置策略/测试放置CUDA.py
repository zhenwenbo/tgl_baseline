...
for i in range(bucket_len):
    ...
    # inDegree = torch.zeros(max(torch.max(src), torch.max(dst)), dtype = torch.int32, device = 'cuda:0')
    # outDegree = torch.zeros_like(inDegree)

    # dgl.sumDegree(inDegree, outDegree, src, dst)

    # # 1. 除去d小于5的，然后将d按从小到大排序，先处理小的放在一起...

    # d = inDegree
    # retain_node = torch.nonzero(d > 1).reshape(-1) #此处是src和dst中的节点id 需要找出>=2的节点id 然后重新获取src和dst
    # retain_mask = torch.zeros(src.shape[0], dtype = torch.bool, device = 'cuda:0')
    # retain_mask[torch.isin()]
    # src = src[torch.isin(src, retain_node)]

    # dg, dg_ind = torch.sort(d, descending=True)

    # # 找到d只为1 只为2的
    # for i in range(torch.max(dg)):
    #     print(f"{i}:{torch.nonzero(dg == i).shape[0]}", end = ' ')


# test = torch.randint(100000000, size=(62567612 * 45,), dtype = torch.int32)