...
class ParallelSampler
{
    public:
                        ...
                        num_history(_num_history), window_duration(_window_duration)
        {
            ...
            //ts_ptr初始化，ts_ptr.shape = [history][node_num], ts_ptr[i][j]表示在历史i的快照上，节点j的第一条边指向节点的索引（即indices的索引）
                           ...
                           std::vector<TimeStampType> &root_ts, float offset)
        {
            ...
            for (std::vector<NodeIDType>::size_type i = 0; i < root_nodes.size(); i++)
            {
                ...
                //同一layer可能有多个相同的源节点但他们的时间戳不同所以采样结果也不同，因此这里加锁。
                omp_set_lock(&(ts_ptr_lock[n]));

                //循环遍历节点i的所有邻边(ts_ptr为起始值，终止值就是最后一条邻边，此处j表示indices中的索引)
                for (std::vector<EdgeIDType>::size_type j = ts_ptr[slc][n]; j < indptr[n + 1]; j++)
                {
                    // std::cout << "comparing " << ts[j] << " with " << root_ts[i] << std::endl;
                    
                    //默认下offset为0，暂不考虑修改duration造成的offset改变
                    //一直循环到在源节点发生时间之前的所有边，最后一条边作为新的ptr更新
                          ...
                          int neighs, bool use_ptr, bool from_root)
        {
            ...
            for (int i = 0; i < num_history; i++)
            {
                ...
                #pragma omp parallel
                {
                    ...
                    //这里的reserve是干嘛的？ 没啥用，cpp特色，预先开空间的
                    ...
                    for (std::vector<NodeIDType>::size_type j = 0; j < (*root_nodes).size(); j++)
                    {
                        ...
                        //对于节点j，算出这个节点的邻域起始和终止。
                        //recent下起始是上一个batch的最终更新的时间戳
                        //非recent下起始就是节点的邻域起始
                        //终止都一样，目的是不超过当前时间戳。
                        //得到s_search和e_search 这两者都是indices的索引。
